...
function solution(s) {
  ...
  for (let i = 0; i < n; ++i) {
    for (let j = 0; j <= i; ++j) {
      ...
      // 设置取更长的回文字符串，遇到更长的就修改start,maxLength 每次都取最长的路径的回文字符串
...
/**
 * 1. dp为n*n的矩阵
 * 2. 计算i到j的字符 是否是回文 是的话 dp[i][j]为true
 * 3. 回文步揍： dp[i-1][j+1] 是回文 且str[i] === str[j]
 * 4. 取每一个回文字符串的最大值 i - j + 1 maxlength
 */
